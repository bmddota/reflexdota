...
function ReflexGameMode:CaptureGameMode()
  if GameMode == nil then
    ...
    ReflexGameMode:CreateTimer("STARTGAME", {
      useGameTime = true,
...
function ReflexGameMode:PlayerSay(keys)
  ...
  if mA ~= nil and mB ~= nil and mC ~= nil and DEBUG then
    ...
    local particle = ParticleManager:CreateParticle("particles/reflex_particles/ref_pudge_meathook_chain.vpcf", PATTACH_RENDERORIGIN_FOLLOW, hooks[1])
      ...
      callback = function(reflex, args)
        ...
        if complete then
          ...
          if diff:Length() > linkCreationTolerance then
            ...
            local particle = ParticleManager:CreateParticle("particles/reflex_particles/ref_pudge_meathook_chain.vpcf", PATTACH_ABSORIGIN, hero)
  ...
  if string.find(text, "^-hook") and DEBUG then
    ...
    local particle = ParticleManager:CreateParticle("particles/reflex_particles/ref_pudge_meathook_chain.vpcf", PATTACH_POINT, hero)
    local position = hero:GetAbsOrigin()
...
function ReflexGameMode:AutoAssignPlayer(keys)
  ...
  callback = function(reflex, args)
    if IsValidEntity(ply) then
      playerID = ply:GetPlayerID()
      if GameRules:State_Get() >= DOTA_GAMERULES_STATE_PRE_GAME and IsValidEntity(ply) and playerID ~= -1 then
        ...
        if heroEntity ~= nil and IsValidEntity(heroEntity) then
          ...
          if GameRules:State_Get() > DOTA_GAMERULES_STATE_PRE_GAME then
            ...
            if heroTable.bRoundInit == false then
              ...
              ReflexGameMode:CreateTimer(DoUniqueString("move"), {
                endTime = GameRules:GetGameTime() + 1,
                useGameTime = true,
                callback = function(reflex, args)
                  FindClearSpaceForUnit(heroTable.hero, heroTable.hero:GetAbsOrigin() + RandomVector(100), true)
                end
              })
      ...
      ply = EntIndexToHScript(entIndex)
    end
...
function ReflexGameMode:InitializeRound()
    ...
    self:LoopOverPlayers(function(player, plyID)
      if player.bRoundInit == false then
        ...
        ReflexGameMode:CreateTimer(DoUniqueString("move"), {
          ...
          useGameTime = true,
          callback = function(reflex, args)
            FindClearSpaceForUnit(player.hero, player.hero:GetAbsOrigin() + RandomVector(100), true)
          end
        })
        --player.hero:RespawnUnit()
  ...
  callback = function(reflex, args)
    ...
    if startCount == 0 then
      reflex.fRoundStartTime = GameRules:GetGameTime()
      self:LoopOverPlayers(function(player, plyID)
...
function ReflexGameMode:Think()
  ...
  -- Process timers
  for k,v in pairs(REFLEX_timers) do
